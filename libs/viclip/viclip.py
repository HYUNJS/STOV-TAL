...
class ViCLIP(nn.Module):
    ...
    def __init__(self,  
        ...
        # self.video_input_num_frames = 16
    ...
    def interpolate_temporal_emb(self, tgt_frame):
        self.video_input_num_frames = tgt_frame
        pos_embed = self.vision_encoder.temporal_positional_embedding.data
        with torch.no_grad():
            # [1, t, d] -> [1, d, t]
            pos_embed = pos_embed.transpose(-2, -1)
            # [1, d, t] -> [1, d, T]
            pos_embed = F.interpolate(pos_embed, size=(tgt_frame), mode='linear', align_corners=True)
            # [1, d, T] -> [1, T, d]
            pos_embed = pos_embed.transpose(-2, -1)
        
        self.vision_encoder.temporal_positional_embedding = nn.Parameter(pos_embed)
