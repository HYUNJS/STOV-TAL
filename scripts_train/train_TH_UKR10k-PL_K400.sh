...
CUDA_VISIBLE_DEVICES=1 python train.py ${cfg_name} ${ckpt} --output ${ver}_th-0.05_load_as_ema --opts dataset.train_json_file ${anno_005} ${ckpt_folder_th005} ${train_cfg} &
CUDA_VISIBLE_DEVICES=3 python train.py ${cfg_name} ${ckpt} --output ${ver}_th-0.10_load_as_ema --opts dataset.train_json_file ${anno_010} ${ckpt_folder_th010} ${train_cfg}
